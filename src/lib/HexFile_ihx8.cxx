 ...
 *
 * $Id: HexFile_ihx8.cxx,v 1.5 2002/10/19 20:32:38 marka Exp $
 */
#include <stdio.h>
#include <stdexcept>
...
HexFile_ihx8::HexFile_ihx8(char *filename, hexfile_mode_t mode)
    : HexFile(filename, mode)
{
    uaddr = 0;
}

HexFile_ihx8::HexFile_ihx8(FILE *fp, hexfile_mode_t mode)
    : HexFile(fp, mode)
{
    uaddr = 0;
}
...
unsigned long addr;
    ...
    while (addr < (unsigned long)start+len) {
        ...
        if (numwords == 0) {
            numwords = 16 / bufwordsize;
        }
        fprintf (
            fp,
            ":%02X%04X00",
            numwords*bufwordsize,
            (unsigned int)(addr*bufwordsize) & 0xffff
        );
        cksum = ((numwords*bufwordsize) & 0xff) +
                (((addr*bufwordsize) >> 8) & 0xff) +
                ((addr*bufwordsize) & 0xff);
        while (numwords > 0) {
