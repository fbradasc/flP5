...
int disassembleInstruction(disassembledInstruction *dInstruction, const assembledInstruction aInstruction) {
	...
	if (AVR_Long_Instruction == AVR_LONG_INSTRUCTION_FOUND) {
		...
		AVR_Long_Address = AVR_Long_Address*2;
	...
	for (instructionIndex = offset; instructionIndex < AVR_TOTAL_INSTRUCTIONS; instructionIndex++) {
		...
		for (i = 0; i < AVR_MAX_NUM_OPERANDS; i++) {
			if (instructionSet[instructionIndex].operandTypes[i] == OPERAND_REGISTER_GHOST) {
				...
				operandTemp = extractDataFromMask(opcode, instructionSet[instructionIndex].operandMasks[0]);
				/* Compare the remaining operands to the first */
				for (j = 1; j < AVR_MAX_NUM_OPERANDS; j++) {
					if (extractDataFromMask(opcode, instructionSet[instructionIndex].operandMasks[i]) !=
							operandTemp)
