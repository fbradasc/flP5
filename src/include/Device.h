 ...
 *
 * $Id: Device.h,v 1.12 2002/11/06 22:42:37 marka Exp $
 */
#ifndef __Device_h
#define __Device_h
...
public:
     ...
     */
    static Device *load(Preferences *cfg,char *name);
     ...
     *        callback function.
     */
    void set_progress_cb (
        bool (*cb)(void *data, long addr, int percent),
        void *data=NULL
    );

    /** Set the current dump callback. The callback will be called
     * every time a word is dumped/disassembled. The callback also
     * returns a boolean value indicating if the operation should continue.
     * This allows a user interface to gracefully stop an operation.
     * \param cb The callback function to set. Set this to NULL to disable
     *        the callback.
...
protected:
    ...
    /** Pointer to a function that will be called after a word is
     * dumped/disassembled.
     * \param data The data pointer that was passed to set_dump_cb().
     * \param line The current dump/disassembled line.
     * \param percent An integer from 0-100 indicating the percentage done.
     * \returns A boolean value indicating if the operation should continue.
     */
    bool (*dump_cb)(void *data, const char *line, int percent);
